...
static int ingest(struct ha_event *ev, const struct ha_device_payload *pl)
{
	...
	ble_record_to_xiaomi(ev->data, (const xiaomi_record_t *)pl->buffer, &ev->timestamp);
...
static const ha_data_descr_t ha_ds_xiaomi_descr[] = {
...
static struct ha_device_endpoint_config ep = {
	...
	.telemetry			   = {.descr	  = ha_ds_xiaomi_descr,
