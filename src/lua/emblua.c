...
#define DECLARE_EMB_LUA_SCRIPT(_name)                                                    \
	...
	STRUCT_SECTION_ITERABLE(emblua,                                                      \
							_name) = {.start = (const char *)&_EMBLUA_START_SYM(_name),  \
...
int lua_fs_populate(void)
{
	...
	STRUCT_SECTION_FOREACH(emblua, script)
	{
		...
		LOG_DBG("Adding file %s to FS, at 0x%x [ %u B ]", script->name, (uint32_t)loc,
				size);
		...
		if (strlen(FS_DEFAULT_DIRECTORY) > 0) {
			ssize_t written = snprintf(path, sizeof(path), "/%s/%s",
									   FS_DEFAULT_MOUNT_POINT, FS_DEFAULT_DIRECTORY);
			...
			if (rc != 0) {
				LOG_ERR("Failed to create directory %s, ret=%d", path, rc);
				goto exit;
			}

			snprintf(&path[written], sizeof(path) - written, "/%s", script->name);
		} else {
			snprintf(path, sizeof(path), "/%s/%s", FS_DEFAULT_MOUNT_POINT, script->name);
		}
