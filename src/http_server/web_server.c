...
bool files_iterate_cb(const char *path,
		      ...
		      void *user_data)
{
	...
	if (dirent->type == FS_DIR_ENTRY_DIR) {
		buffer_snprintf(buf, "<li><b><a href=\"/fetch%s\">%s</a></b></li>",
				abspath, abspath);
	} else {
		buffer_snprintf(buf, "<li><a href=\"/files%s\">%s</a> (size = %u)</li>",
				abspath, abspath, dirent->size);
	}
