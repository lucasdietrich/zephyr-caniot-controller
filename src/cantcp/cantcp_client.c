...
{
	return (tunnel->server.hostname != NULL) &&
		(tunnel->server.hostname[0] != '\0');
}
...
static int host_resolve(cantcp_tunnel_t *tunnel)
{
	...
	if (ret != 0) {
		LOG_ERR("(%x) failed to resolve hostname (%s) err = %d",
			(uint32_t)tunnel, tunnel->server.hostname, ret);
		goto exit;
	}
...
int cantcp_connect(cantcp_tunnel_t *tunnel)
{
	...
	int sock = zsock_socket(tunnel->server.addr4.sin_family,
				SOCK_STREAM, IPPROTO_TCP);
	if (sock < 0) {
		LOG_ERR("(%x) Failed to create socket = %d", 
	...
	ret = zsock_connect(sock, &tunnel->server.addr,
			    sizeof(tunnel->server.addr));
	if (ret < 0) {
		LOG_ERR("(%x) Failed to connect to server %s:%d = %d",
			(uint32_t)tunnel, ipv4_str, tunnel->server.port, ret);
